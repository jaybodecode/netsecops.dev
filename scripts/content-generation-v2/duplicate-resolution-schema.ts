...
export const UpdateObjectSchema = z.object({
  datetime: z.string().describe(`
    ...
    **ONLY fill this if decision is UPDATE/SKIP-UPDATE.**
  ...
  sources: z.array(z.object({
    url: z.string().describe("Source URL from the NEW article"),
    title: z.string().describe("Source title/headline from the NEW article")
  })).describe(`
    Sources from the NEW article that report this update.
    Include ONLY 1-3 most relevant sources for this specific update.
    Do NOT duplicate sources from the original article.
    **ONLY fill this if decision is UPDATE/SKIP-UPDATE.**
    Leave empty array for NEW or SKIP decisions.
  `),
